...
const CustomizeOrderPage: React.FC = () => {
  ...
  const [acceptableToppings, setAcceptableToppings] =
    useState<ToppingOption[]>();
  ...
  const selectedStyleChanged = (style: string) => {
    ...
    if (s) {
      ...
      if (style === 'cold' || style === 'blended') {
        ...
        setAcceptableToppings(
          toppings?.filter((e) => e.nameEn !== 'chocolate sauce')
        );
      } else {
        setAcceptableToppings(toppings);
        if (sizes) {
          var _newSizes = sizes.filter(
            (e) => e.nameEn === 'S' || e.nameEn === 'M'
          );
  ...
  const calculate = (
    menu: MenuOption | null | undefined,
    style: StyleOption | null | undefined,
  ...
  const addToCartC = () => {
    ...
    if (
      menuOption.availableSizes !== undefined &&
      menuOption.availableSizes.length > 0
    ) {
      if (!selectedSize) {
    ...
    if (
      menuOption.availableStyles !== undefined &&
      menuOption.availableStyles.length > 0
    ) {
      if (!selectedSize) {
      ...
      <div className="container">
        <div className="columns is-desktop">
          ...
          <div className="column">
                ...
                <div className="column is-two-fifths">
                  <div className="is-flex is-flex-direction-row is-justify-content-flex-start-mobile">
                    <QuantitySelector
                      max={99}
                      min={1}
                      onQuantityChanged={(value) => quanlityChanged(value)}
                    ></QuantitySelector>
                  </div>
                </div>
              </div>
                  ...
                  {styles?.map((e) => (
                    <p className="control" key={e.nameEn}>
                      <button
                        ...
                        onClick={() => selectedStyleChanged(e.nameEn)}
                      >
                  ...
                  {acceptableSizes
                    ...
                    .map((e) => (
                      <p className="control" key={e.nameEn}>
                        <button
                          className={`button ${
                            e.nameEn === selectedSize?.nameEn
                              ? 'is-primary'
                              : ''
                          }`}
                          onClick={() => selectedSizeChanged(e.nameEn)}
                        >
                          {language === 'en' ? e.nameEn : e.nameVi}
                        </button>
                      </p>
            ...
            {/* TOPPING SELECTOR */}
            {menuOption?.availableToppings?.length! > 0 && (
              <div className="block">
                <p className="is-size-5 has-text-weight-semibold pb-3">
                  Toppings
                </p>
