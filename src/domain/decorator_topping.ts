...
class ToppingDecorator extends OrderDecorator {
  ...
  constructor(
    orderBase: ItemBase,
    _topping: ToppingOption,
    _toppingCount: number
  ) {
    super(orderBase);
  ...
  getDescription(): string {
    let result: string = `${this.decoratedOrder.getDescription()}, ${
      true ? this.topping.nameEn : this.topping.nameVi
    }`;
    if (this.count > 1) {
      result += `(${this.count})`;
    }
  ...
  getPrice(): number {
    return (
      this.decoratedOrder.getPrice() + this.topping.getCountPrice(this.count)
    );
  }
}
